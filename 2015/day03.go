...
func main() {
	...
	// ^ moves him north, v moves him south, > moves him east and < moves him west

	// examples:
	// ^>v< delivers presents to 4 houses in a square,
	// including twice to the house at his starting location

	// Part One
	// Question: How many houses receive at least one present?
	fmt.Printf("%s houses receive at least one present\n", partOne())

	// Part Two
	// Question: With Robo-Santa together, how many houses receive at least one present?
	fmt.Printf("With Robo-Santa %s houses receive at least one present\n", partTwo())
}

func partOne() string {
	coordinates := []int{0, 0}                // X, Y
	housesThatHavePresents := []string{"0/0"} // Santa always drops a present at the beginning
	for i := range strings.Count(input(), "") - 1 {
		if string(input()[i]) == "^" {
			coordinates[0]++
		} else if string(input()[i]) == "v" {
			coordinates[0]--
		...
		currentCoordinate := fmt.Sprint(coordinates[0], "/", coordinates[1])
		if !slices.Contains(housesThatHavePresents, currentCoordinate) {
			housesThatHavePresents = append(housesThatHavePresents, currentCoordinate)
		}
	}
	return fmt.Sprint(len(housesThatHavePresents))
}

func partTwo() string {
	coordinatesSanta := []int{0, 0}           // X, Y of Santa
	coordinatesRoboSanta := []int{0, 0}       // X, Y of Robo-Santa
	housesThatHavePresents := []string{"0/0"} // Santa and Robo-Santa drop a present at the beginning
	for i := range strings.Count(input(), "") - 1 {
		if i%2 == 0 { // Santa moves
			...
			currentCoordinateSanta := fmt.Sprint(coordinatesSanta[0], "/", coordinatesSanta[1])
			if !slices.Contains(housesThatHavePresents, currentCoordinateSanta) {
				housesThatHavePresents = append(housesThatHavePresents, currentCoordinateSanta)
			}

		} else { // Robo-Santa moves
			if string(input()[i]) == "^" {
				coordinatesRoboSanta[0]++
			} else if string(input()[i]) == "v" {
				coordinatesRoboSanta[0]--
			} else if string(input()[i]) == ">" {
				coordinatesRoboSanta[1]++
			} else if string(input()[i]) == "<" {
				coordinatesRoboSanta[1]--
			}
			currentCoordinateRoboSanta := fmt.Sprint(coordinatesRoboSanta[0], "/", coordinatesRoboSanta[1])
			if !slices.Contains(housesThatHavePresents, currentCoordinateRoboSanta) {
				housesThatHavePresents = append(housesThatHavePresents, currentCoordinateRoboSanta)
			}
		}
